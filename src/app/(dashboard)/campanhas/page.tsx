...
export default function CampanhasPage() {
  const [activeTab, setActiveTab] = useState<'campanhas' | 'configuracao'>('campanhas')
  ...
  useEffect(() => {
    async function fetchConfigDisparo() {
      ...
      if (res.ok) {
        ...
        if (data) {
          setConfigDisparo({
            twilio_numero_origem: data.twilio_numero_origem || 'whatsapp:+14155238886',
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Gerencie campanhas e configurações de disparo WhatsApp</p>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '28px', background: 'var(--bg-card)', borderRadius: '10px', padding: '4px', width: 'fit-content', border: '1px solid var(--border)' }}>
        {[
          { id: 'campanhas', label: 'Campanhas', icon: '📢' },
          { id: 'configuracao', label: 'Configuração de Disparo', icon: '⚙️' }
        ].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as 'campanhas' | 'configuracao')}
            style={{ padding: '8px 18px', borderRadius: '7px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500', fontFamily: 'DM Sans, sans-serif', background: activeTab === tab.id ? 'var(--accent)' : 'transparent', color: activeTab === tab.id ? '#fff' : 'var(--text-secondary)', transition: 'all 0.15s' }}>
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'configuracao' && (
        <div style={{ maxWidth: '600px' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '28px', marginBottom: '16px' }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)', marginBottom: '20px', marginTop: 0 }}>Conexão Twilio</h2>

            <div style={{ marginBottom: '20px' }}>
              <span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '600',
                background: configDisparo.twilio_modo === 'producao' ? '#2dd4a020' : '#f5c84220',
                color: configDisparo.twilio_modo === 'producao' ? '#2dd4a0' : '#f5c842' }}>
                {configDisparo.twilio_modo === 'producao' ? '🟢 Produção' : '🟡 Sandbox'}
              </span>
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontFamily: 'DM Sans, sans-serif' }}>Número de origem</label>
              <input type="text" value={configDisparo.twilio_numero_origem} placeholder="whatsapp:+14155238886"
                onChange={e => setConfigDisparo(prev => ({ ...prev, twilio_numero_origem: e.target.value }))}
                style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 14px', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontFamily: 'DM Sans, sans-serif' }}>Modo de operação</label>
              <select value={configDisparo.twilio_modo} onChange={e => setConfigDisparo(prev => ({ ...prev, twilio_modo: e.target.value as 'sandbox' | 'producao' }))}
                style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 14px', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'DM Sans, sans-serif' }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '28px', marginBottom: '16px' }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)', marginBottom: '20px', marginTop: 0 }}>Regras de Disparo</h2>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontFamily: 'DM Sans, sans-serif' }}>Horário de início</label>
                <input type="number" min={0} max={23} value={configDisparo.twilio_horario_inicio}
                  onChange={e => setConfigDisparo(prev => ({ ...prev, twilio_horario_inicio: +e.target.value }))}
                  style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 14px', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontFamily: 'DM Sans, sans-serif' }}>Horário de fim</label>
                <input type="number" min={0} max={23} value={configDisparo.twilio_horario_fim}
                  onChange={e => setConfigDisparo(prev => ({ ...prev, twilio_horario_fim: +e.target.value }))}
                  style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 14px', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' }} />
              </div>
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px', fontFamily: 'DM Sans, sans-serif' }}>Limite diário de disparos</label>
              <input type="number" min={1} value={configDisparo.twilio_limite_diario}
                onChange={e => setConfigDisparo(prev => ({ ...prev, twilio_limite_diario: +e.target.value }))}
                style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 14px', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' }} />
            </div>
          </div>

          <button onClick={salvarConfigDisparo} disabled={savingConfig}
            style={{ width: '100%', padding: '12px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', fontFamily: 'DM Sans, sans-serif', cursor: savingConfig ? 'not-allowed' : 'pointer', opacity: savingConfig ? 0.7 : 1 }}>
