...
function FormField({
  label, value, onChange, type = 'text', options, col,
}: {
  label: string
  value: string
...
export default function AdminPage() {
  ...
  const kpis = [
    { label: 'Total Escritórios', value: tenants.length,                                 color: '#4f7aff', icon: <Building2 size={16} /> },
    { label: 'Ativos',            value: tenants.filter(t => t.status === 'ativo').length, color: '#22c55e', icon: <CheckCircle size={16} /> },
    { label: 'Em Trial',          value: tenants.filter(t => t.status === 'trial').length, color: '#f5c842', icon: <Clock size={16} /> },
    { label: 'Leads Totais',      value: metricas.totalLeads.toLocaleString('pt-BR'),      color: '#a855f7', icon: <Users size={16} /> },
    { label: 'Conversas',         value: metricas.totalConversas.toLocaleString('pt-BR'),  color: '#2dd4a0', icon: <MessageSquare size={16} /> },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080b14', fontFamily: 'DM Sans, sans-serif', color: '#fff' }}>
      ...
      <div style={{ height: '56px', background: '#111827', borderBottom: '1px solid #1f2937', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <button
          onClick={logout}
          style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#9ca3af', background: 'none', border: '1px solid #1f2937', borderRadius: '8px', padding: '6px 12px', cursor: 'pointer' }}
        >
          <LogOut size={13} /> Sair
        </button>
      </div>
      ...
      <div style={{ padding: '28px', maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '14px', marginBottom: '28px' }}>
          {kpis.map(k => (
            <div key={k.label} style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: '12px', padding: '16px 18px' }}>
              ...
              <p style={{ fontSize: '26px', fontWeight: '700', color: '#fff', fontFamily: 'Syne, sans-serif', margin: 0 }}>{k.value}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', gap: '12px' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '7px', background: 'linear-gradient(135deg, #4f7aff, #7c3aed)', border: 'none', borderRadius: '10px', padding: '9px 18px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}
          >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #1f2937' }}>
                {['Escritório', 'Responsável / OAB', 'Plano', 'Status', 'Cadastro', 'Ações'].map(h => (
            ...
            <tbody>
              ...
              {filtrados.map((t, i) => {
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: '1px solid #1f2937', background: i % 2 === 0 ? 'transparent' : '#0d1117' }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ background: `${p.color}20`, color: p.color, border: `1px solid ${p.color}40`, borderRadius: '20px', padding: '3px 10px', fontSize: '11px', fontWeight: '600' }}>{p.label}</span>
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ background: `${s.color}20`, color: s.color, border: `1px solid ${s.color}40`, borderRadius: '20px', padding: '3px 10px', fontSize: '11px', fontWeight: '600' }}>{s.label}</span>
                    </td>
      ...
      {showForm && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '560px', maxHeight: '90vh', overflowY: 'auto', background: '#111827', border: '1px solid #1f2937', borderRadius: '16px', padding: '28px', zIndex: 1000 }}>
