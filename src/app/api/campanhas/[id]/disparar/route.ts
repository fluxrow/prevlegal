...
function buildMessage(template: string, lead: any): string {
  return template
    ...
    .replace(/\{ganho\}/gi, lead.ganho_potencial ? `R$ ${Number(lead.ganho_potencial).toFixed(2)}` : '')
}

async function sendWhatsApp(to: string, message: string): Promise<{ sid: string } | null> {
  ...
  const res = await fetch(
    `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`,
    {
      ...
      body: new URLSearchParams({ From: `whatsapp:${from.replace('whatsapp:', '')}`, To: `whatsapp:${to}`, Body: message }).toString()
    }
  )
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!['rascunho', 'pausada'].includes(campanha.status)) {
      return NextResponse.json({ error: 'Campanha já está ' + campanha.status }, { status: 400 })
    }

    // Marcar como ativa
    await adminClient.from('campanhas').update({ status: 'ativa', iniciado_em: new Date().toISOString() }).eq('id', campanhaId)

    // Buscar leads da lista
    const query = adminClient
      .from('lista_leads')
      .select('lead_id, leads(id, nome, nb, cpf, banco, valor_rma, ganho_potencial, tem_whatsapp)')
      .eq('lista_id', campanha.lista_id)

    const { data: listaLeads } = await query

    if (!listaLeads || listaLeads.length === 0) {
      await adminClient.from('campanhas').update({ status: 'concluida', concluido_em: new Date().toISOString() }).eq('id', campanhaId)
      return NextResponse.json({ success: true, enviados: 0, message: 'Nenhum lead na lista' })
    }

    // Filtrar leads: apenas com WhatsApp verificado se configurado
    const leads = listaLeads
      .map((ll: any) => ll.leads)
      .filter((l: any) => l && (!campanha.apenas_verificados || l.tem_whatsapp === true))
    ...
    if (!numero) {
      await adminClient.from('campanhas').update({ status: 'rascunho' }).eq('id', campanhaId)
      return NextResponse.json({ error: 'Nenhum número WhatsApp ativo disponível' }, { status: 400 })
    }
    ...
    for (let i = 0; i < leadsParaEnviar.length; i++) {
      ...
      const { data: status } = await adminClient
        .from('campanhas').select('status').eq('id', campanhaId).single()
      ...
      if (!phone) {
        await adminClient.from('campanha_mensagens').insert({
          campanha_id: campanhaId, lead_id: lead.id,
          numero_whatsapp_id: numero.id, telefone: null,
        ...
        if (result) {
          await adminClient.from('campanha_mensagens').insert({
            campanha_id: campanhaId, lead_id: lead.id,
            numero_whatsapp_id: numero.id, telefone: phone,
            mensagem: mensagem, status: 'enviado',
            twilio_sid: result.sid, enviado_at: new Date().toISOString()
          })
          enviados++
          await adminClient.from('numeros_whatsapp')
            .update({ total_enviados_hoje: jaEnviados + enviados, ultimo_envio_at: new Date().toISOString() })
            .eq('id', numero.id)
        } else {
          await adminClient.from('campanha_mensagens').insert({
            campanha_id: campanhaId, lead_id: lead.id,
      ...
      if (i % 10 === 0) {
        await adminClient.from('campanhas').update({ total_enviados: enviados, total_falhos: falhos }).eq('id', campanhaId)
      }
    ...
    // Finalizar campanha
    await adminClient.from('campanhas').update({
      status: 'concluida',
