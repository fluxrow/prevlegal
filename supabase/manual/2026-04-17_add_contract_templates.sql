-- PrevLegal
-- Patch manual para templates de contrato e bucket de minutas
-- Data: 2026-04-17
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'tenant members can view their contract templates'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'contract_templates'
      and policyname = 'tenant admins can manage their contract templates'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'storage'
      ...
      and policyname = 'Authenticated users can upload contract docs'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'storage'
      and tablename = 'objects'
      and policyname = 'Authenticated users can read contract docs'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'storage'
      and tablename = 'objects'
      and policyname = 'Authenticated users can delete contract docs'
  ) then
